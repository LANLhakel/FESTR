...
Mesh::Mesh(void): nzones(0), zone() {}

//-----------------------------------------------------------------------------

Mesh::Mesh(const size_t nin): nzones(0), zone() {zone.reserve(nin);}
...
{
    nzones = 0;
    for_each(zone.begin(), zone.end(), utils::DeleteObject());
    zone.clear();
}

//-----------------------------------------------------------------------------

void Mesh::add_zone(const Zone * const z) {zone.push_back(z); ++nzones;}

//-----------------------------------------------------------------------------

const Zone * Mesh::get_zone(const size_t i) const {return zone.at(i);}

//-----------------------------------------------------------------------------

size_t Mesh::size(void) const {return nzones;}
...
FaceID Mesh::next_face(const Grid &g, const RetIntercept &h) const
{
    ...
    {
        const Zone *z = get_zone(0);    // bounding Sphere Zone
        const Face *f = z->get_face(1); // inner Surface of *z
    ...
    {    // or exiting bounding Sphere Zone through its outer Face (Sphere)
        const Zone *z = get_zone(h.fid.my_zone);  // this Zone
        const Face *f = z->get_face(h.fid.my_id); // exit Face of *z
        size_t nf = f->num_nbr();
        for (size_t i = 0; i < nf; ++i)
        {
            ...
            {
                const Zone *nz = get_zone(0);
                const Face *fz = nz->get_face(1);
                const Surface *fs = dynamic_cast<const Surface *>(fz);
                const Face *fn = fs->get_face(fid.my_zone);
                if (fn->contains(g, h.w)) return FaceID(0, 1);
            }
            else // the neighbor is a regular Zone
            {
                const Zone *nz = get_zone(fid.my_zone);
                const Face *fn = nz->get_face(fid.my_id);
...
void Mesh::load(const std::string &path, const std::string &tlabel)
{
    ...
    for (size_t i = 0; i < nzones; ++i)
    {
        ...
        zone.push_back(new Zone(geometry, material));
    }
...
std::string Mesh::to_string(void) const
{
    std::string s("Number_of_zones");
    s += utils::int_to_string(nzones, ' ', cnst::INT_WIDTH);
    s += cnst::ZONE_SEPARATOR;
    for (size_t i = 0; i < nzones; ++i)
    {
        s += zone.at(i)->to_string();
        if (i < nzones-1) s += cnst::ZONE_SEPARATOR;
    }
    return s;
}
