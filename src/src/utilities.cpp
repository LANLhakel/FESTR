...
double utils::radians(const double x) {return x * (cnst::PI / 180.0);}

//-----------------------------------------------------------------------------

double utils::degrees(const double x) {return x * (180.0 / cnst::PI);}

//-----------------------------------------------------------------------------

int utils::nint(const double x) {return static_cast<int>(floor(x + 0.5));}
...
std::string utils::trim(std::string str) {return trim_char(str, ' ');}
                             ...
                             const std::vector<double> &zb)
{
    ...
    // accumulate, lambda, etc. in lieu of the for-loop are possible here
    return s * np;
}
                                    ...
                                    const bool add_zero)
{
    ...
    if (add_zero)
    {
        ...
        if ( *(rv.end()-1) < 0.0) rv.push_back(0.0);
    }
...
std::string utils::bool_to_string(const bool b) {return b ? "true" : "false";}
