#ifndef LANL_ASC_PEM_OBJECTIVE_H_
#define LANL_ASC_PEM_OBJECTIVE_H_
...
public:
     ...
     * @return String
     */
    std::string is_x_present() const;

    /**
     * @brief Getter for Objective::w_is_present in string form
     * @return String
     */
    std::string is_w_present() const;
    ...
    void update_best(const T &a, const size_t n, const size_t j)
    {
        ...
        if (rescale) // scale "a", if data "y" units are unknown
        {
            ...
            for (size_t i = 0; i < n; ++i) {ascaled[i] = scale * a[i];}
...
#endif  // LANL_ASC_PEM_OBJECTIVE_H_
